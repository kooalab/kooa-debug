...
async function u(t, e = {}) {
  ...
  try {
    ...
    if (d2)
      throw this.hooks.call("fetch:timeout", n2, { url: t }), new c(`Request timed out: ${t}`, { url: t, timedOut: d2 });
    if ("AbortError" === (null == e2 ? void 0 : e2.name) || h2.aborted)
      throw new c(`Request aborted: ${t}`, { url: t, aborted: true });
    throw e2;
  }
  const { status: m2, url: g2 } = u2, f2 = await u2.text();
  if (500 === m2)
    throw this.hooks.call("fetch:error", n2, { status: m2, response: u2, url: g2 }), new c(`Server error: ${g2}`, { status: m2, url: g2 });
  if (!f2)
    throw new c(`Empty response: ${g2}`, { status: m2, url: g2 });
  const { url: v2 } = l.fromUrl(g2), w2 = { url: v2, html: f2 };
  return !n2.cache.write || e.method && "GET" !== e.method || t !== v2 || this.cache.set(w2.url, w2), w2;
}
var d = class {
  constructor(t) {
...
var P = class {
  ...
  async call(t, e, i2, s2) {
    ...
    const [c2] = await this.run(l2, n2, o2);
  ...
  async run(t, e, i2) {
    ...
    for (const { hook: n2, handler: o2, defaultHandler: r2, once: a2 } of t) {
      if (null != e && e.done)
        continue;
      a2 && this.off(n2, o2);
      const t2 = await v(o2, [e || this.swup.visit, i2, r2]);
      s2.push(t2);
    }
    return s2;
  }
  runSync(t, e, i2) {
    ...
    for (const { hook: n2, handler: o2, defaultHandler: r2, once: a2 } of t) {
      ...
      s2.push(t2), f(t2) && console.warn(`Promise returned from handler for synchronous hook '${n2}'.Swup will not wait for it to resolve.`);
    }
    return s2;
  }
  getHandlers(t, e) {
...
var C = "transition";
var $ = "animation";
async function x({ elements: t, selector: e }) {
  if (false === e && !t)
    return;
  let i2 = [];
  if (t)
  ...
  const s2 = i2.map((t2) => function(t3) {
    const { type: e2, timeout: i3, propCount: s3 } = function(t4, e3) {
      const i4 = window.getComputedStyle(t4), s4 = A(i4, `${C}Delay`), n2 = A(i4, `${C}Duration`), o2 = H(s4, n2), r2 = A(i4, `${$}Delay`), a2 = A(i4, `${$}Duration`), l2 = H(r2, a2);
      let h2 = null, c2 = 0, u2 = 0;
      return c2 = Math.max(o2, l2), h2 = c2 > 0 ? o2 > l2 ? C : $ : null, u2 = h2 ? h2 === C ? n2.length : a2.length : 0, { type: h2, timeout: c2, propCount: u2 };
    }(t3);
    return !(!e2 || !i3) && new Promise((n2) => {
      ...
      const l2 = () => {
        ...
        if (e3.target === t3) {
          if (!function(t4) {
            return [`${C}end`, `${$}end`].includes(t4.type);
          }(e3))
...
async function V(t, e = {}) {
  if (this.navigating) {
    ...
    await this.hooks.call("visit:abort", this.visit, void 0), this.visit.state = 8;
  }
  ...
  var h2, c2;
  l2 && (t.animation.name = l2), "object" == typeof e.cache ? (t.cache.read = null != (h2 = e.cache.read) ? h2 : t.cache.read, t.cache.write = null != (c2 = e.cache.write) ? c2 : t.cache.write) : void 0 !== e.cache && (t.cache = { read: !!e.cache, write: !!e.cache }), delete e.cache;
  try {
    ...
    if (this.currentPageUrl = n(), t.history.popstate && this.classes.add("is-popstate"), t.animation.name && this.classes.add(`to-${s(t.animation.name)}`), t.animation.wait && await i3, t.done)
      return;
...
var T = async function(t) {
  ...
  await g(), await this.hooks.call("animation:in:start", t, void 0, () => {
...
var N = async function(t, e) {
  ...
  t.advance(6);
  const { url: i2 } = e;
  this.classes.remove("is-leaving"), this.isSameResolvedUrl(n(), i2) || (r(i2), this.currentPageUrl = n(), t.to.url = this.currentPageUrl), t.animation.animate && this.classes.add("is-rendering"), await this.hooks.call("content:replace", t, { page: e }, (t2, { page: e2 }) => {
  ...
  }), await this.hooks.call("content:scroll", t, void 0, () => this.scrollToContent(t)), await this.hooks.call("page:view", t, { url: this.currentPageUrl, title: document.title });
};
var O = function(t) {
  var e;
...
//# sourceMappingURL=chunk-MFRXIDBF.js.map
